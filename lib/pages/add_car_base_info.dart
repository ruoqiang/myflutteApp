...
//  var _inviteController;
  ...
  goToNextPage2() {
    if (_uploadImage1== null) {
      return showToast("请上传行驶证正面照片");
    }
    if (_carColorController.text == '') {
      return showToast("请输入车牌颜色");
    }
    if (_carNumController.text == '') {
      return showToast("请输入车牌号");
    }

    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) {
        return AddCarOtherInfoPage();
      }),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          _formList(_labelAndInput('车牌颜色:', '请输入车牌颜色', _carColorController)),
          _formList(_labelAndInput('车牌号:', '请输入车牌号', _carNumController)),
          _formList(_labelAndInput('发动机号:', '请输入发动机号', _nameController)),
          _formList(_labelAndInput('车辆识别代码:', '请输入车辆识别代码', _nameController)),
